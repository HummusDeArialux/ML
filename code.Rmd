...
```{python}
# Definimos una función que nos ayude a contar las instancias
...
for i in k_list:
  ...
  y_pred2 = knn_i.predict_proba(seq_encoded_test2)[:, 1]  # predicted probabilities of class 1
  fpr, tpr, thresholds = roc_curve(y2_test, y_pred2, pos_label = "1")  # calculate fpr, tpr, and thresholds
  roc_auc = roc_auc_score(y2_test, y_pred2.astype(str))  # calculate ROC AUC
  
  accuracy_i = knn_i.score(seq_encoded_test2, y2_test)
  kappa_i = cohen_kappa_score(y2_test, y_pred2)
  cm_i = confusion_matrix(y2_test, y_pred2, labels=["1", "0"])
  order_i = knn_i.classes_
  error_i = 1 - knn_i.score(seq_encoded_test2, y2_test)
  
  results_list2.append([accuracy_i, kappa_i, error_i, cm_i, order_i])
